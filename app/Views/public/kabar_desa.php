...
<section class="max-w-6xl mx-auto px-4 sm:px-6 py-20 md:py-28">
    ...
    <div class="text-center mb-16 anime-fade-up opacity-0">
        ...
        <p class="text-earth-700 max-w-2xl mx-auto">Dokumentasi kegiatan, momen spesial, dan potret kehidupan Desa Lubuk
            Lagan.</p>
    </div>

    <?php if (empty($blogs)): ?>
        <div class="text-center py-20 text-earth-400">
            <svg class="w-16 h-16 mx-auto mb-4 text-earth-200" fill="none" stroke="currentColor" viewBox="0 0 24 24">
    ...
    <?php else: ?>
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-7">
            <?php foreach ($blogs as $blog): ?>
                <article
                    class="bg-white rounded-[2rem] shadow-sm border border-earth-100 overflow-hidden hover:shadow-2xl hover:-translate-y-1 transition-all duration-300 anime-card opacity-0 group">
                    ...
                    <div class="relative overflow-hidden h-52 bg-earth-100">
                        ...
                        <?php if ($thumb): ?>
                            <img src="<?= esc($thumb) ?>" alt="<?= esc($blog->title) ?>"
                                class="w-full h-full object-cover group-hover:scale-105 transition-transform duration-500"
                                loading="lazy">
                        <?php else: ?>
                            <div class="w-full h-full flex items-center justify-center">
                                <svg class="w-12 h-12 text-earth-200" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5"
                                        d="M19 11H5m14 0a2 2 0 012 2v6a2 2 0 01-2 2H5a2 2 0 01-2-2v-6a2 2 0 012-2m14 0V9a2 2 0 00-2-2M5 11V9a2 2 0 012-2m0 0V5a2 2 0 012-2h6a2 2 0 012 2v2M7 7h10" />
                                </svg>
                            </div>
                    ...
                    <div class="p-6">
                        <h3
                            class="font-heading font-bold text-forest-900 text-lg leading-tight mb-2 group-hover:text-earth-700 transition">
                            <?= esc($blog->title) ?></h3>
                        ...
                        <div class="mt-4 pt-4 border-t border-earth-50 flex items-center justify-between">
                            <time
                                class="text-xs text-earth-400 font-medium"><?= date('d M Y', strtotime($blog->published_at ?? $blog->created_at)) ?></time>
                            <a href="<?= base_url('baca/' . $blog->slug) ?>"
                                class="text-earth-500 text-xs font-semibold flex items-center gap-1 hover:text-earth-700 transition">
                                Baca <svg class="w-3 h-3 transform group-hover:translate-x-0.5 transition" fill="none"
